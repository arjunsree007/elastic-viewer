...
function ElasticConnection(props) {
	...
	function handleEgClick(event) {
		...
		setFormValues({
			...formValues,
			url: 'http://poc.molecularconnections.com/elastic-service/',
		});
		// "http://139.162.210.220:9201/" });
	}
		...
		<div className='bg-light card mt-2 card-body'>
				...
				<Form>
					<Form.Row>
						<Form.Group as={Col} md='3' controlId='url'>
							...
							<Form.Text
								className={
									formErrors.url === '' ? 'text-muted ' : 'text-danger'
								}>
								{formErrors.url === ''
						...
						<Form.Group as={Col} md='3' controlId='core'>
							...
							<Form.Text
								className={
									formErrors.core === '' ? 'text-muted ' : 'text-danger'
								}>
								{formErrors.core === ''
							...
							<Form.Label>
								<Link
									to='/'
									onClick={(event) => handleCoreListModalOpen(event)}>
									List of cores
								</Link>{' '}
								{loadingCoreList && (
			...
			{coreList && coreList.length > 0 && (
				<CoreListModal
					...
					coreList={coreList}></CoreListModal>
			)}
		</div>
	);
}
